...
def sleep_with_heartbeat(tasks_collection, task_id: str, seconds: int) -> bool:
    ...
    while time.time() < end_time:
        # Heartbeat
        tasks_collection.update_one({"_id": ObjectId(task_id)}, {"$set": {"updated_at": now_iso()}})
        ...
        if stop_requested or (end_at_dt and current_time >= end_at_dt):
            ...
            tasks_collection.update_one({"_id": ObjectId(task_id)}, {"$set": {"status": status, "stopped_at": now_iso(), "updated_at": now_iso()}})
...
def run_task_worker(task_id: str, shared_store: Optional["Dict[str, Any]"] = None) -> None:
    ...
    if not use_hub:
        ...
        if video_capture is None:
            ...
            if (task.get("source_type") == "file") or (not task.get("source_uri") and bool(task.get("file_path"))):
                tasks_collection.update_one({"_id": ObjectId(task_id)}, {"$set": {"status": "completed", "stopped_at": now_iso(), "updated_at": now_iso()}})
    ...
    try:
        ...
        if run_mode == "continuous":
            ...
            while True:
                # stop/end checks - support both old and new field names
                task_document = tasks_collection.find_one({"id": task_id}, projection={"stop_requested": 1, "end_time": 1, "end_at": 1})
                ...
                if stop_requested or (end_at_dt and current_time >= end_at_dt):
                    status = "completed" if end_at_dt and current_time >= end_at_dt else "cancelled"
                    tasks_collection.update_one({"id": task_id}, {"$set": {"status": status, "stopped_at": now_iso(), "updated_at": now_iso()}})
                    print(f"[worker {task_id}] ⏹️ Stopping (status={status}, end_at={end_at_dt}, now={current_time})")
                    return
                ...
                if use_hub:
                    ...
                    if not is_frame_read:
                        ...
                        if source_is_file:
                            tasks_collection.update_one({"id": task_id}, {"$set": {"status": "completed", "stopped_at": now_iso(), "updated_at": now_iso()}})
